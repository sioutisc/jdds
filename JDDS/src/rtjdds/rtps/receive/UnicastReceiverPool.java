...
import rtjdds.rtps.transport.UDPUnicastReceiver;
 ...
 *
 */
public class UnicastReceiverPool extends ReceiverPool {
	...
	public UnicastReceiverPool(int nLanes, int domainId, Database db) throws IOException {
		...
		for (int i=0; i<nLanes; i++) {
			/* receiver (the socket) */
			// TODO parametrizzare !!
			UDPUnicastReceiver receiver = new UDPUnicastReceiver(
					new InetSocketAddress("127.0.0.1",7401), GlobalProperties.BUFFER_SIZE);
//			int priority = minPrio + step * i;
//			_lanes[i] = new ReceiverThread(receiver,db,priority);
			_lanes[i] = new ReceiverThread(receiver,db,RealtimeThread.MAX_PRIORITY);
		}
