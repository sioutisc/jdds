...
public class CDRByteArrayInputPacket extends InputPacket {
	...
	/* indicates if the cursor has reached the end of the buffer */
	private boolean bufferFull = false;
	
	/**
	 * Constructs a new <code>CDRInputPacket</code> over the given buffer
	 * of bytes, with the given length (<i><=buffer.length</i>) and the
	 * given endianess.
	 * @param buffer
	 * @param length
	 * @param isLittleEndian
	 */
	public CDRByteArrayInputPacket(byte[] buffer, int length, boolean isLittleEndian) {
		this.buffer = buffer;
		this.limit = length;
	 ...
	 * @param isLittleEndian
	 */
	public CDRByteArrayInputPacket(byte[] buffer, boolean isLittleEndian) {
		this.buffer = buffer;
	 ...
	 * @param buffer
	 */
	public CDRByteArrayInputPacket(byte[] buffer) {
		this.buffer = buffer;
		this.limit = buffer.length;
	 ...
	 * @return true if the position of the cursor is allowed
	 */
...
//				 the message will be truncated and no more writes are possible
	...
	public byte read_octet() {
		byte retVal = buffer[currPos];
		// TODO (leaky...)
		if (checkBounds()) {currPos++;}
		return retVal;
	}
